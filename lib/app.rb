...
def say_hello_on_twitter(array, client)
  array.sample(5).each do |handle|
    client.update("Thanks for being so accurate #{handle} #bonjour_monde @the_hacking_pro")
  end
end

#client.search("#bonjour_monde").take(25).each do |tweet|
def favorite_on_twitter(client, tweet)
  client.favorite(tweet)
end

def follow_on_twitter(client, tweet)
...
streaming.filter(track: "#bonjour_monde") do |tweet|
  client.favorite(tweet)
  client.follow(tweet.user)
  puts "Nous avons liké et suivi un nouvel utilisateur !"
end